        ...
        cart.length===0 ? (
          ...
          <div className="row">
            <div className="col-md-12 text-center">
                <h2 className="display-3 text-warning">Cart is empty</h2>
            </div>
            </div>
            </div>
          ...
          <div className="row">
            <div className="col-md-12 text-center">
                <h2 className="display-3 text-warning">Cart</h2>
            </div>
            </div>
            <div className="row">
                      ...
                      cart && cart.map((item,index)=>{
                        return(
                          <tr key={index}>
                            <td>{item.title}</td>
                            <td>
                              <img src={item.thumbnail} alt="no image found" width={80} height={80} />
                            </td>
                            <td>&#8377;{item.price}</td>
                            <td>
                              <div className="d-flex justify-content-evenly">
                                <i onClick={()=>
                                  decrement(item.id)} className="bi bi-dash-circle text-danger pointer"></i>
                                <strong>{item.quantity}</strong>
                                <i onClick={()=>
                                  increment(item.id)} className="bi bi-plus-circle text-danger  pointer"></i>
                              </div>
                            </td>
                            <td>&#8377;{item.price*item.quantity}</td>
                            <i onClick={()=>{
                              delItem(item.id)
                            }} className="bi bi-trash text-danger pointer"></i>
                          </tr>
                        )
                      })
                    }
